...
def file_based_input_fn_builder(input_file, seq_length, is_training,
    ...
    name_to_features = {
        ...
        "label_ids": tf.FixedLenFeature([4], tf.int64),
...
train_data_path='reforcement_test.csv'
...
x_test = test[:100] #testing a small sample
x_test = x_test.reset_index(drop=True)
predict_examples = create_examples(x_test,False)        # False是 label_available
...
# train和val都是使用的file_based_convert_examples_to_features，因为train和val读入的都是文件类型，
# predicte使用样本作为传入，不是用文件传入的，所以使用convert_exmaples_to_features
# 输出的 InputFeatures 对象。 函数中: mask标注，padding， 开头[CLS]和结尾[SEP]
